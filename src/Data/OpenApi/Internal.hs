...
import Data.OpenApi.Aeson.Compat        (deleteKey, keyToText, objectToList, objectFromList)
...
newtype SpecificationExtensions = SpecificationExtensions {getSpecificationExtensions :: InsOrdHashMap Text Value}
...
instance FromJSON Responses where
  parseJSON =
    withObject "Responses" $ \o ->
      let (extensions, rest) = partitionObject isExt $ deleteKey "default" o
      in Responses
        <$> o .:? "default"
        <*> parseJSON (Object rest)
        <*> parseJSON (Object extensions)
    where
      isExt = Text.isPrefixOf "x-" . keyToText
      partitionObject p =
        bimap objectFromList objectFromList . List.partition (isExt . fst) . objectToList
...
instance FromJSON SpecificationExtensions where
  parseJSON =
    withObject "SpecificationExtensions" $
      pure . SpecificationExtensions . InsOrdHashMap.fromList . filterExtFields . objectToList
    where
      filterExtFields = mapMaybe (\(k, v) -> (, v) <$> Text.stripPrefix "x-" (keyToText k))
